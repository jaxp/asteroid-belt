...
import com.pallas.service.captcha.utils.FilePathUtils;
...
public class SlidingCaptcha extends AbstractCaptcha {
    ...
    // 底部图片后缀
    private static final String BASE_SUFFIX = "_base.png";
    // 方块图片后缀
    private static final String CUTOUT_SUFFIX = "_cutout.png";
    // 最终图片长度
    ...
    @Override
    public CaptchaResult init(String cid) {
        try {
            SlidingParams param = this.loadImage(cid);
            this.generateBlock(param);
            this.cutImage(param);
            return new CaptchaResult(param.getResult(),
                new StringBuilder(param.getStartX() + "").append(PlsConstant.COLON)
                    .append(param.getWidth()).append(PlsConstant.COLON)
                    .append(UUID.randomUUID()).toString());
        } catch (Exception e) {
            throw new RuntimeException("滑动验证码初始化失败", e);
        }
    }
    ...
    private SlidingParams loadImage(String cid) throws IOException {
    ...
    private void cutImage(SlidingParams param) throws IOException {
        ...
        try {
            ...
            // 保存图片
            File baseFile = new File(FilePathUtils.getFilePath(param.getCid() + BASE_SUFFIX));
            File cutoutFile = new File(FilePathUtils.getFilePath(param.getCid() + CUTOUT_SUFFIX));
            if (!baseFile.getParentFile().exists()) {
                baseFile.getParentFile().mkdirs();
            }
            ImageIO.write(blurBaseImage, "png", baseFile);
            ImageIO.write(blurCutoutImage, "png", cutoutFile);
            //CompressUtils.fromBufferedImage2(blurBaseImage, "jpg");
            Map<String, Object> result = new HashMap<>();
            result.put("base", baseFile.getName());
            result.put("cutout", cutoutFile.getName());
            result.put("cid", param.getCid());
            result.put("rate", LENGTH);
            result.put("y", 0.5 - LENGTH - LENGTH * RADIUS);
